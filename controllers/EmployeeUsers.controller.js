...
module.exports.loginUser = catchAsync(async function (req, res, next) {
  ...
  const user = await mongoose
    ...
    .findOne(
      { email: body.email },
      "name email password isConfirmed isPasswordSet"
    )
  ...
  if (!user.isConfirmed)
    return next(new AppError("Your Access is pending", 403));
...
module.exports.startTracking = catchAsync(async function (req, res, next) {
  ...
  const setLocationGeoPoint = new GeoPoint(
  ...
  if (distance > location.radius)
    return next(
      new AppError(
        `You are ${(distance - location.radius).toFixed(
          2
    ...
    if (!logOfDay) {
      ...
      for (let index = 0; index < logOfDay.length; index++) {
        flag =
          ...
          logOfDay[index].hasOwnProperty("out");
        if (flag == false) break;
      }
      if (flag == true) {
...
module.exports.remove = catchAsync(async function (req, res, next) {
  ...
  for (const id of ids) {
    if (!mongoose.isValidObjectId(id))
      return next(new AppError("Please enter valid id(s)", 400));
  }
...
module.exports.getTimeSheet = catchAsync(async function (req, res, next) {
  ...
  const timesheet = mergedLoggedHours.map((mergedLoggedHour) => ({
    employee: employees.find(
      (e) => e._id.toString() === mergedLoggedHour.employee.toString()
    ),
...
module.exports.exportTimesheet = catchAsync(async function (req, res, next) {
  ...
  mergedLoggedHours.forEach((e) => {
    const populatedEmployee = employees.find(
      (employee) => employee._id.toString() === e.employee.toString()
    );
    Object.entries(e.logs).forEach(([key, value]) => {
      value.forEach((interval) => {
        ...
        if (time == "24")
          row["Clock In"] = dayjs("1/1/1 " + interval["in"]).format("H:mm");

        if (interval["out"]) {
          row["Clock Out"] = interval["out"];

          if (time == "24")
            row["Clock Out"] = dayjs("1/1/1 " + interval["out"]).format("H:mm");
        }
  ...
  if (format === "csv") {
    const json2csv = new Parser({
      fields: ["Date", "Day", "Id", "Name", "Clock In", "Clock Out"],
    });
